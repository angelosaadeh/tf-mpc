...
def est3_mpc(mpc, v, iterations, target):
    ...
    for it in range(iterations):
        ...
        y = mpc.mul(pos + neg, n)
        y.shape = (nq, 1)
        y = mpc.normalize(y, amin, amax, free=False, unitary=True)
        ...
        diff.shape = (nq, 1)
        diff = mpc.normalize(diff, amin, amax, free=False, unitary=True)
        ...
        trust.shape = (1, nv)
        trust = mpc.normalize(trust.transpose(), amin, amax, free=False, unitary=True).transpose()
        p.append(time.time()-tempp)
        q.append(time.process_time()-tempq)
...
def function(mpc, v):
    ...
    print('-------MPC model--------')
    a = est3_mpc(mpc, v.transpose(), 5, target)
    print('-------Base model--------')
    b = est3_clear(v_clear.transpose(), 5, target)

    print('Questions labelled the same way: ', np.sum(((a[0] - 0.5) * (b[0] - 0.5)) > 0))
